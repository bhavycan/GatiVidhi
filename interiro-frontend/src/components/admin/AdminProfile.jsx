...
const Field = ({ icon, label, value, editing, name, onChange, disabled = false }) => (
...
const AdminProfile = () => {
  ...
  useEffect(() => {
    axios.get(`${API_BASE}/admin/profile-info`, { withCredentials: true })
      .then(({ data }) => {
        setAdmin(data.admin)
  ...
  const handleSave = async () => {
    ...
    setSaveMsg('')
    try {
      const { data } = await axios.post(`${API_BASE}/admin/update-self`, { name: formName }, { withCredentials: true })
      setAdmin(prev => ({ ...prev, name: data.admin.name }))
      setEditing(false)
  ...
  const handleSendOtp = async () => {
    ...
    try {
      await axios.post(`${API_BASE}/admin/send-otp`, {}, { withCredentials: true })
      setOtpSent(true)
  ...
  const handleChangePassword = async () => {
    ...
    setPwdMsg({ text: '', error: false })
    try {
      await axios.post(`${API_BASE}/admin/verify-change-password`, { otp, newPassword }, { withCredentials: true })
            ...
            <div className='w-full flex items-center mt-[4%] pb-4 border-b-2 border-white justify-between'>
              <div className='flex items-center gap-3'>
                <div className='w-12 h-12 rounded-full bg-[#f3a9de] border-2 border-[#883bbc] flex items-center justify-center shrink-0'>
                  <i className='ri-shield-user-line text-2xl text-[#883bbc]'></i>
                </div>
                <div>
                  <p className='font-bold text-base leading-tight'>{admin.name || '—'}</p>
                  <p className='text-xs text-white opacity-70 font-medium'>{admin.email}</p>
                  <span className='text-xs bg-[#883bbc] text-white px-2 py-0.5 rounded-full font-semibold mt-0.5 inline-block'>Admin</span>
                </div>
              </div>
            </div>
          </header>
          ...
          <div className='relative z-0 mb-4'>
            ...
            <div className='bg-white rounded-2xl border border-black px-5 py-5 flex flex-col gap-4'>
              <div className='flex items-center justify-between mb-1'>
                ...
                {!editing
                  ? <button onClick={() => { setFormName(admin.name); setEditing(true); setSaveMsg('') }}
                      className='flex items-center gap-1 text-sm font-semibold text-[#883bbc] hover:opacity-70 transition-opacity'>
                      <i className='ri-pencil-line'></i> Edit
                    </button>
                  : <div className='flex gap-3'>
                      <button onClick={() => setEditing(false)} className='text-sm font-semibold text-gray-400 hover:opacity-70'>Cancel</button>
              ...
              <Field icon='ri-user-line' label='Full Name' name='name'
                value={editing ? formName : admin.name} editing={editing}
                onChange={e => setFormName(e.target.value)} />
              <Field icon='ri-mail-line' label='Email Address' name='email'
                value={admin.email} editing={false} disabled onChange={() => {}} />
                      ...
                      {!otpSent ? (
                        <div className='flex flex-col gap-3'>
                          <p className='text-sm text-gray-500 leading-5'>
                            We'll send a 6-digit OTP to <strong>{admin.email}</strong> to verify your identity.
                          </p>

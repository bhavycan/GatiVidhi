...
const AdminDashboard = () => {
    ...
    const convert = moment("12-25-1995", "MMMM Do YYYY").format('LL');
   const convertday = moment("12-25-1995", "MMMM Do YYYY").format('dddd');
        ...
        <div className='max-w-3xl mx-auto'>
          ...
          <section className='completion-data w-full mt-[5%]'>
            ...
            <div className='card-container mt-[2%] w-full relative flex items-center justify-between gap-4'>
              <div className="card w-[45%] md:w-5/12 bg-gradient-to-br backdrop-blur-lg from-white to-transparent relative rounded-md shadow-2xl shadow-current h-32 md:h-40"></div>
              <div className="card w-[45%] md:w-5/12 bg-gradient-to-br backdrop-blur-lg from-white to-transparent shadow-2xl shadow-current rounded-md h-32 md:h-40"></div>
            </div>
          </section>
          ...
          <section className='daily-update-log w-full mt-[5%]'>
            ...
            <div ref={parent} className="update-log-cards mt-[4%] flex flex-col justify-center max-h-[40vh] overflow-y-auto">
              {Array.from({length: 10}).map((item, index) => {
                return (
                  <div key={index} className="update-card px-[2%] mt-[5%] py-[1%] bg-gradient-to-r from-white to-transparent w-full h-20 md:h-16 rounded-md flex-shrink-0 shadow-xl backdrop-blur-lg flex items-center justify-center">
                    <div className='name w-[40%] h-full flex items-center justify-center'>
                      <h4 className='text-lg leading-5'>Malbar Exotica</h4>
                    </div>
                    <div className='name w-[60%] flex items-end flex-col justify-center h-full'>
                      <h4 className='text-base h-6 flex items-center justify-center font-bold'>{convert}</h4>
                      <h4 className='text-base h-6 flex items-center justify-center font-bold'>{convertday}</h4>
                    </div>
                  </div>
                )
              })}
